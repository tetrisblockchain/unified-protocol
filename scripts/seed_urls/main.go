...
const (
	defaultSeedRPCURL        = "http://localhost:3337"
	...
	defaultSeedPreflightTimeout = 12 * time.Second
...
type seedPreflightConfig struct {
	Enabled   bool
...
func main() {
	...
	flag.BoolVar(&preflightEnabled, "url-preflight", envOrDefaultBool("UFI_SEED_PREFLIGHT", true), "preflight URLs before quoting and submission; skips obvious HTTP/network failures")
	flag.DurationVar(&preflightTimeout, "url-preflight-timeout", envOrDefaultDuration("UFI_SEED_PREFLIGHT_TIMEOUT", defaultSeedPreflightTimeout), "HTTP timeout for URL preflight checks")
	flag.Parse()
	...
	privateKey, err := parseSeedKey(seedKeyFromEnv())
	if err != nil {
		logger.Fatal(err)
	}
	...
	entries, skipped, err := filterSeedEntries(logger, entries, seedPreflightConfig{
	...
	if skipped > 0 {
		logger.Printf("preflight skipped %d URL(s)", skipped)
	}
	if len(entries) == 0 {
		logger.Fatalf("no seedable URLs remain after preflight filtering")
	}
...
func filterSeedEntries(logger *log.Logger, entries []seedFileEntry, cfg seedPreflightConfig) ([]seedFileEntry, int, error) {
	if !cfg.Enabled {
		return append([]seedFileEntry(nil), entries...), 0, nil
	}

	timeout := cfg.Timeout
	if timeout <= 0 {
		timeout = defaultSeedPreflightTimeout
	}
	...
	filtered := make([]seedFileEntry, 0, len(entries))
	skipped := 0
	for _, entry := range entries {
		status, err := preflightSeedURL(client, cfg.UserAgent, entry.URL)
		if err != nil {
			skipped++
			if logger != nil {
				logger.Printf("preflight skip url=%s err=%v", entry.URL, err)
			}
			continue
		}
		if status >= http.StatusBadRequest {
			skipped++
			if logger != nil {
				logger.Printf("preflight skip url=%s status=%d", entry.URL, status)
			}
			continue
		}
...
func preflightSeedURL(client *http.Client, userAgent, rawURL string) (int, error) {
	if client == nil {
		client = &http.Client{Timeout: defaultSeedPreflightTimeout}
	}
...
func parseSeedKey(raw string) (ed25519.PrivateKey, error) {
	cleaned := strings.TrimSpace(raw)
	if cleaned == "" {
		return nil, fmt.Errorf("UFI_SEED_KEY, UFI_OPERATOR_KEY, or UFI_ARCHITECT_KEY is required")
	}
	...
	if decoded, err := hex.DecodeString(strings.TrimPrefix(cleaned, "0x")); err == nil {
		...
		return nil, fmt.Errorf("seed key must be hex or base64 encoded")
	}
	...
	default:
		return nil, fmt.Errorf("seed key must decode to %d-byte seed or %d-byte private key", ed25519.SeedSize, ed25519.PrivateKeySize)
	}
}

func seedKeyFromEnv() string {
	for _, key := range []string{"UFI_SEED_KEY", "UFI_OPERATOR_KEY", "UFI_ARCHITECT_KEY"} {
