...
RELEASE_NETWORK_CONFIG="${RELEASE_NETWORK_CONFIG:-$ROOT_DIR/config/unified-network.mainnet.json}"
...
package_desktop_bundle() {
  local desktop_dir="$ROOT_DIR/web3/desktop"
  local platform
  local bundle_name
  local stage_dir
  local artifact_path
...
package_website_bundle() {
  ...
  rm -rf "$stage_dir"
  mkdir -p "$stage_dir"
  cp -R "$ROOT_DIR/Website" "$stage_dir/Website"
  cp "$ROOT_DIR/README.md" "$stage_dir/README.md"
...
package_ops_bundle() {
  ...
  if [ "$RELEASE_INCLUDE_CONTRACTS" = "1" ]; then
    ...
    if [ -f "$ROOT_DIR/contracts_UNS_sol_UNS.bin" ]; then
      cp "$ROOT_DIR/contracts_UNS_sol_UNS.bin" "$stage_dir/contracts/"
    fi
    if [ -f "$ROOT_DIR/contracts_UNS_sol_IUFDToken.bin" ]; then
      cp "$ROOT_DIR/contracts_UNS_sol_IUFDToken.bin" "$stage_dir/contracts/"
    fi
