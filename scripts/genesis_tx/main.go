...
func main() {
	...
	if err := validateExpectedArchitectAddress(client, architectAddress.String()); err != nil {
		logger.Fatal(err)
	}
...
func validateExpectedArchitectAddress(client *rpcClient, derived string) error {
	expected := strings.TrimSpace(os.Getenv("UFI_ARCHITECT_ADDRESS"))
	if expected == "" && client != nil {
		if network, err := client.networkConfig(); err == nil {
			candidate := strings.TrimSpace(network.ArchitectAddress)
			if candidate != "" && !strings.Contains(candidate, "REPLACE_ME") {
				expected = candidate
			}
		}
	}
...
func (c *rpcClient) networkConfig() (core.NetworkConfig, error) {
	var result core.NetworkConfig
	err := c.call("ufi_getNetworkConfig", map[string]any{}, &result)
	return result, err
}
