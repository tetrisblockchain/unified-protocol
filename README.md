...
- `web/desktop`: Electron + React desktop wallet/explorer for local key management, transfers, UNS registration, and crawl-ledger visibility.
- `web/site`: static website and live dashboard assets for browser-side exploration.
- `web/governance`: RPC client bindings consumed by the CLI and UI surfaces.
- `config/networks`: pinned network manifests and manifest examples for launch packaging.
...
- `contracts/artifacts`: compiled contract outputs kept out of the repo root.
- `archive/`: legacy scaffolds and retired scripts that are intentionally excluded from the active build graph.
...
A pinned mainnet manifest example is in [unified-network.mainnet.example.json](/Users/efrainvera/Documents/UNIFIED/config/networks/unified-network.mainnet.example.json). Generate your real manifest from it, do not hand-edit per-node values into different copies.
...
```bash
go run ./cmd/unified-node \
  --network-config ./config/networks/unified-network.local.json \
  --datadir ./data \
  --rpcport 3337 \
  --mine \
...
- RPC stays on `127.0.0.1:3337` unless you change `UNIFIED_RPC_HOST`.
- Browser CORS is disabled by default. Set `UNIFIED_RPC_CORS_ORIGINS=https://your-site.example` or `*` if you intentionally want public browser access.
...
Package a versioned mainnet release bundle with runtime binaries, ops assets, website files, and the desktop distribution bundle:
...
Artifacts are written under `./build/release/<version>/` with `SHA256SUMS`, `release-manifest.json`, and a top-level `unified-mainnet-launch-<version>.tar.gz`.
...
The desktop app is in `web/desktop`. It derives UFI addresses locally from an Ed25519 seed, signs raw transactions inside the Electron preload context, resolves UNS names, registers new UNS names, and shows live chain/search/governance state from the connected node.
...
The default RPC endpoint is `http://127.0.0.1:3337`. You can change it inside the app without restarting. The wallet key is session-local to the desktop process and is not posted to the node.
...
UFI_RPC_URL=http://127.0.0.1:3337 \
...
Example `testdata/seeds/urls.txt`:
...
The seeder preflights URLs before quoting by default, skipping obvious HTTP/network failures like persistent `403` targets before they ever hit the mempool. It then uses the current `/consensus/quote` rules per URL, enqueues tasks in sender-safe batches, and waits until the tasks are mined. With the current sender limit, one account can keep at most `32` tasks in flight at once.
...
```bash
curl -X POST http://127.0.0.1:3337/governance/proposals \
  -H 'Content-Type: application/json' \
  -d '{
...
```bash
curl -X POST http://127.0.0.1:3337/chain/advance \
  -H 'Content-Type: application/json' \
  -d '{"blocks":40321}'

curl -X POST http://127.0.0.1:3337/governance/finalize \
  -H 'Content-Type: application/json' \
  -d '{"proposalId":1}'
```
...
```bash
curl http://127.0.0.1:3337/governance/rules
```

Quote a governance-adjusted crawl bounty:

```bash
curl -X POST http://127.0.0.1:3337/consensus/quote \
  -H 'Content-Type: application/json' \
  -d '{
